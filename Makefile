PROJECT_NAME :=  Triton Package
include build/common.mk

PACK             := triton
PACKDIR          := sdk
PROJECT          := github.com/Smithx10/pulumi-triton
NODE_MODULE_NAME := @pulumi/triton
...
build::
	go install -ldflags "-X github.com/Smithx10/pulumi-triton/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}
	go install -ldflags "-X github.com/Smithx10/pulumi-triton/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
...
install::
	GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/Smithx10/pulumi-triton/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
